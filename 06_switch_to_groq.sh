...
# --- Edit 1: .env.example — swap Anthropic key for Groq key ---
sed -i.bak \
  -e 's/# Anthropic — the agent loop itself/# Groq — the agent loop itself/' \
  -e 's/ANTHROPIC_API_KEY=/GROQ_API_KEY=/' \
  .env.example
rm -f .env.example.bak
echo "edited .env.example (ANTHROPIC_API_KEY -> GROQ_API_KEY)"

# --- Edit 2: registry.py — TOOL_SCHEMAS restructured to Groq/OpenAI format ---
...
Schemas are in Groq's function-calling format (OpenAI-compatible):
...
EOF
echo "rewrote agent/tools/registry.py (Groq function-calling format)"
